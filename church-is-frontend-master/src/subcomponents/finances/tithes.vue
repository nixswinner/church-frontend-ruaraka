...
export default {
  ...
  setup(props, { emit }) {
    ...
    const tithe_amount_error = reactive([])
    const tithe_member_error = reactive([])
    const added_tithe = reactive([])
    const add_tithe_errors = reactive([])
    ...
    const all_member_ids = reactive([])

    const sms_status = reactive([])
    ...
    const onGroupSelected = (groupValue) => {
    ...
    const getTithes = () => {
      fetch_data_error.length = 0
      store.dispatch('update_isLoading', true)

      tithes.value = JSON.parse(localStorage.getItem('tithe_list'))
      if (tithes.value) {
        const array = tithes.value.response
        foundTithes.value = array.length
        array.forEach(tithe => {
          all_member_ids.push(tithe.id)
        })
        emitToParent()
        store.dispatch('update_isLoading', false)
      }

      tithe_stats.value = JSON.parse(localStorage.getItem('tithe_stats'))

      const currentVersion = store.getters.tithe_list_version
      const version = localStorage.getItem('tithe_list_version')

      if (!version || version < currentVersion) {
        all_members.value = false
        member_ids.value = []
        all_member_ids.length = 0
        let params = {}
        if (from_date.value && to_date.value) {
          params = { from_date: from_date.value, to_date: to_date.value }
        }
        store.dispatch('update_isLoading', true)
        fetch(`${store.state.baseURL}/api/finance/tithes-by-members/`, { method: 'GET', params })
          .then(response => response.json())
          .then(data => {
            tithes.value = { response: data }
            data.forEach(tithe => {
              all_member_ids.push(tithe.id)
            })
            emitToParent()
            foundTithes.value = data.length
            localStorage.setItem('tithe_list', JSON.stringify({ response: data }))
            localStorage.setItem('tithe_list_version', currentVersion)
            store.dispatch('update_isLoading', false)
          })
          .catch(err => {
            fetch_data_error.push(err)
            store.dispatch('update_isLoading', false)
          })
      }
    ...
    const getTitheStats = () => {
      fetch(`${store.state.baseURL}/api/finance/tithe-stats/`, { method: 'GET' })
        .then(response => response.json())
        .then(data => {
          tithe_stats.value = { response: data }
          localStorage.setItem('tithe_stats', JSON.stringify({ response: data }))
        })
        .catch(err => {
          fetch_data_error.push(err)
        })
    ...
    const addTitheFormOK = () => {
      added_tithe.length = 0
      tithe_member_error.length = 0
      tithe_amount_error.length = 0
      if (tithe_narration.value.length < 1) {
        tithe_narration.value = "none given"
      }
      if (!tithe_date.value) {
        alert('date is required')
        return false
      }
      if (tithe_amount.value < 1) {
        alert("enter an amount")
        return false
      }
      return true
    ...
    const addTithe = () => {
      if (addTitheFormOK()) {
        enable_add_tithe_button.value = false
        add_tithe_button_text.value = 'adding tithe...'
        adding_tithe.value = true
        fetch(`${store.state.baseURL}/api/finance/add-tithe-for-member/`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            member: selectedMember.value,
            narration: tithe_narration.value,
            recorded_by: sessionStorage.getItem('member_id'),
            amount: tithe_amount.value,
            date: tithe_date.value,
          ...
          .then(response => response.json())
          .then(data => {
            adding_tithe.value = false
            selectedMember.value = null
            tithe_narration.value = ''
            tithe_amount.value = ''
            tithe_date.value = null
            payment_method.value = null

            let new_version = parseInt(localStorage.getItem('tithe_list_version')) + 1
            store.dispatch('update_tithe_list_version', new_version)
            getTithes()

            alert("tithe successfully added, amount: " + data.amount)
          })
          .catch(err => {
            adding_tithe.value = false
            alert(err)
    ...
    const exportData = () => {
      ...
      fetch(`${store.state.baseURL}/api/finance/get-tithes-as-csv/${csv_date.value}/`, { method: 'GET' })
        .then(response => response.blob())
        .then(blob => {
          const url = window.URL.createObjectURL(blob)
          const a = document.createElement('a')
          a.href = url
          a.download = `tithes_${csv_date.value}.csv`
