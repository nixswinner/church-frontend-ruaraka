...
export default {
  ...
  setup() {
    ...
    const groups_selected = ref(false);
    const church_groups = ref(null);
    const finances_selected = ref(false);
    const contribution_info = ref(null);
    const pledges_info = ref(null);
    const offering_info = ref(null);
    const tithe_info = ref(null);
    const tithe_stats = ref(null);
    const offering_stats = ref(null);
    const family_tree = ref(null);
    ...
    const deleting_member = ref(false);

    const getMemberInfo = async () => {
      try {
        const response = await axios.get(
          `${window.$BASE_URL}/api/members/member/${route.params.id}/`
        );
    ...
    const getMemberGroups = async () => {
      ...
      groups_selected.value = true;
      try {
        const response = await axios.get(
          `${window.$BASE_URL}/api/groups/church-groups-for-a-member/${route.params.id}/`
        );
        church_groups.value = { response: response.data };
    ...
    const getMemberFinances = async () => {
      store.dispatch("update_isLoading", true);
      finances_selected.value = true;
      try {
        const [
          contributions,
          pledges,
          ...
          axios.get(
            `${window.$BASE_URL}/api/projects/contributions-by-member/${route.params.id}/`
          ),
          axios.get(
            `${window.$BASE_URL}/api/projects/pledges-by-member/${route.params.id}/`
          ),
          axios.get(
            `${window.$BASE_URL}/api/finance/tithe-stats-for-member/${route.params.id}/`
          ),
          axios.get(
            `${window.$BASE_URL}/api/finance/tithe-for-member/${route.params.id}/`
          ),
          axios.get(
            `${window.$BASE_URL}/api/finance/offering-stats-for-member/${route.params.id}/`
          ),
          axios.get(
            `${window.$BASE_URL}/api/finance/offerings-by-member/${route.params.id}/`
          ),
        ]);

        contribution_info.value = { contribution: contributions.data };
        pledges_info.value = { pledges: pledges.data };
        tithe_stats.value = { stats: titheStats.data };
        tithe_info.value = { tithes: tithes.data };
        offering_stats.value = { stats: offeringStats.data };
        offering_info.value = { offerings: offerings.data };
      } catch (error) {
      } finally {
        store.dispatch("update_isLoading", false);
      }
    ...
    const getRoles = async () => {
      ...
      try {
        const response = await axios.get(
          `${window.$BASE_URL}/api/members/role-list/`
        );
        roles.value = { response: response.data };
      } catch (error) {
        fetch_data_error.value.push(error);
      }
    };

    const getRoleForMember = async () => {
      try {
        const response = await axios.get(
          `${window.$BASE_URL}/api/members/roles-for-member/${route.params.id}/`
        );
        roles_for_member.value = { response: response.data };

        let count = Object.keys(roles_for_member.value.response).length;
        if (count > 0) {
          member_has_roles.value = true;
        }
      } catch (error) {
        fetch_data_error.value.push(error);
      }
    ...
    const assignRoles = async () => {
      ...
      try {
        await Promise.all(
          selected_role.value.map((roleId) =>
            axios.post(
              `${window.$BASE_URL}/api/members/add-role-for-member/`,
              {
    ...
    const deleteMember = async () => {
      deleting_member.value = true;
      try {
        await axios.delete(
          `${window.$BASE_URL}/api/members/member/${route.params.id}/`
        );
        deleting_member.value = false;
        alert("member deleted");
        localStorage.removeItem("member_list_version");
        localStorage.removeItem("member_list");
        location.reload();
